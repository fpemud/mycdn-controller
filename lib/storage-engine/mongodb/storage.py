...
class StorageEngine:
    ...
    def start(self):
        ...
        for msId, msParam in self._mirrorSiteDict.items():
            ...
            if self._mongoServer is None:
                self._mongoServer = _MultiInstanceMariadbServer(self._listenIp, self._tmpDir, self._logDir)
                self._mongoServer.start()
        self._bStart = True
    ...
    def stop(self):
        ...
        self._bStart = False
        if self._mongoServer is not None:
            self._mongoServer.stop()
            self._mongoServer = None
    ...
    def advertiseMirrorSite(self, mirrorSiteId):
        assert self._bStart
        self._mongoServer.exportDatabase(mirrorSiteId)
