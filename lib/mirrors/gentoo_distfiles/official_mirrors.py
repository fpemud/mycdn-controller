...
class MirrorRefreshThread(threading.Thread):
    ...
    def run(self):
        url = "https://api.gentoo.org/mirrors/distfiles.xml"        # from app-portage/mirrorselect-2.2.2
        ...
        for mirrorgroup in ElementTree.XML(text):
            for mirror in mirrorgroup:
                ...
                for e in mirror:
                    ...
                    if e.tag == 'uri':
                        if e.get("protocols") not in ["http", "https", "ftp", "rsync"]:
                            continue
                        if e.get("ipv4") != "y":
                            continue
                        obj.uriDict[e.get("protocol")] = e.text
                self.mirror_cb(obj)
...
class _MirrorObject:
    ...
    def __init__(self):
        ...
        self.uriDict = dict()       # to be filled in MirrorObjectFactory
        self.api = None
